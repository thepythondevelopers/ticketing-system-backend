...
exports.getCategoryData = (req,res)=>{
    ...
    PlanCategory.find({user:req.user._id}).exec((err,category)=>{
        if(err){
            return res.status(400).json({
...
exports.getSingleCategoryData = (req,res)=>{
    ...
    PlanCategory.find({user:req.user._id,_id:req.params.id}).exec((err,category)=>{
        if(err){
            return res.status(400).json({
                message : "No Data Found"
            })
